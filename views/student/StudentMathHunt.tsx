...
export const StudentMathHunt: React.FC<StudentMathHuntProps> = ({
  ...
  const generateProblem = useCallback(
    async (currentLevel: number) => {
      ...
      if (grade.startsWith("1.") || grade.startsWith("2."))
        gradeMultiplier = 0.5;
      else if (grade.startsWith("3.") || grade.startsWith("4."))
        gradeMultiplier = 1;
      else if (
        grade.startsWith("5.") ||
        grade.startsWith("6.") ||
      ...
      if (
        ...
        switch (effectiveTopic) {
          ...
          case "multiplication": {
            ...
            const mMax1 =
              gradeMultiplier < 1
                ? 5
                : (currentLevel + 2) * Math.sqrt(gradeMultiplier);
            const mMax2 =
              gradeMultiplier < 1 ? 5 : 10 * Math.sqrt(gradeMultiplier);
          ...
          case "division":
            ...
            ans =
              Math.floor(
                Math.random() * (currentLevel + 2) * Math.sqrt(gradeMultiplier),
              ) + 1;
            num1 = num2 * ans;
            q = `${num1} ÷ ${num2}`;
            break;
        ...
        try {
          const aiProblem = await generateMathProblem(
            topic,
            currentLevel,
            grade,
          );
  ...
  const generateReport = async () => {
    ...
    try {
      const res = await generateMathReport(
        nickname,
        playerScore,
        level,
        session.config?.topic || "matematikk",
      );
      setReport(res);
    } catch (e) {
      setReport(
        "Klarte ikke å generere rapporten. Men du gjorde en kjempeinnsats!",
      );
    } finally {
  ...
  const handleGetHint = async () => {
    ...
    try {
      const h = await generateMathHint(
        problem.q,
        answer,
        session.config?.topic || "matematikk",
      );
  ...
  const handleSelectLevel = (lvl: number) => {
    ...
    supabase
      .from("quiz_players")
      .update({ last_answer: lvl.toString() })
      .eq("id", playerId)
      .then(() => onSync());
  };

...
export const MagicLinkForm: React.FC<MagicLinkFormProps> = ({ t, onSuccess, onBack }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email) return;
    setIsLoading(true);
    const { error } = await storageService.signInWithMagicLink(email);
    setIsLoading(false);
    if (error) {
      addToast(error.message, 'error');
    } else {
      addToast(t.magicLinkSent || "Magisk lenke sendt til e-posten din.", 'success');
      onSuccess();
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="p-4 bg-indigo-50 rounded-2xl inline-flex text-indigo-600 mb-2 shadow-sm"><Mail size={24} /></div>
      <h2 className="text-2xl font-black text-slate-900 uppercase tracking-tight">{t.magicLinkTitle}</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button type="submit" disabled={isLoading} className="w-full bg-indigo-600 text-white p-5 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-indigo-700 active:scale-95 disabled:opacity-50 flex items-center justify-center shadow-lg shadow-indigo-200 mt-4">
          {isLoading ? <Loader2 className="animate-spin" /> : <><Send size={16} className="mr-2" /> {t.sendMagicLink}</>}
        </button>
      </form>
    </div>
  );

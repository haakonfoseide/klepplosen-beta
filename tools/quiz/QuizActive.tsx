  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 rounded-[3rem] overflow-hidden relative">
      ...
      <div className="bg-slate-900 text-white p-4 flex justify-between items-center px-8 shadow-md relative z-10">
        <span className="font-black text-xs uppercase tracking-widest text-indigo-300">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-sm">{players.length}</span>
      ...
      <div className="flex-grow flex flex-col items-center justify-center p-8 text-center gap-8 bg-white relative overflow-hidden">
          ...
          <div className="absolute left-6 top-1/2 -translate-y-1/2 w-56 hidden xl:flex flex-col gap-2 animate-in slide-in-from-left duration-700 z-10">
            ...
            {players
              ...
              .map((p, i) => (
                <div
                  key={p.id}
                  className="bg-slate-50 border border-slate-100 p-3 rounded-2xl flex items-center justify-between shadow-sm animate-in slide-in-from-left"
                  style={{ animationDelay: `${i * 100}ms` }}
                >
                  <div className="flex items-center gap-3">
                    <span
                      className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black ${i === 0 ? "bg-amber-400 text-white" : "bg-slate-200 text-slate-500"}`}
                    >
                      {i + 1}
                    </span>
                    <span className="font-bold text-xs truncate max-w-[80px] text-slate-700">
                      {p.nickname.split(" ").slice(1).join(" ")}
                    </span>
                  </div>
                  <span className="font-black font-mono text-[10px] text-purple-600">
                    {p.score}
                  </span>
                </div>
              ))}
        ...
        {gameStatus === "question" ? (
          <>
            <h2 className="text-3xl sm:text-5xl font-black text-slate-900 leading-tight max-w-4xl">
              {q.question}
            </h2>
          ...
          <div className="flex flex-wrap justify-center gap-2 max-w-4xl">
            {players.map((p) => {
              ...
              return (
                <div
                  key={p.id}
                  className={`px-3 py-1.5 rounded-full font-black text-[9px] uppercase tracking-widest transition-all border-2 flex items-center gap-2 ${hasAnswered ? "bg-emerald-500/10 border-emerald-500 text-emerald-600" : "bg-slate-50 border-slate-100 text-slate-300"}`}
                >
        ...
        {/* Options Grid */}
        <div
          className={`grid ${q.type === "true-false" ? "grid-cols-2" : "grid-cols-1 sm:grid-cols-2"} gap-4 w-full max-w-5xl mt-4 ${isTimeUp ? "opacity-50 pointer-events-none" : ""}`}
        >
          {q.options.map((opt, i) => {
            ...
            return (
              <div
                key={i}
                className={`p-8 rounded-[2rem] text-white font-black text-xl sm:text-2xl shadow-lg transition-all flex items-center gap-4 ${bg} ${isTimeUp && !isCorrect ? "opacity-20" : "opacity-100"} ${isTimeUp && isCorrect ? "scale-105 shadow-xl ring-4 ring-emerald-300" : ""}`}
              >
      ...
      {gameStatus === "reveal" && (
        <div className="absolute bottom-8 right-8 z-50 animate-in slide-in-from-bottom flex gap-4">
          <button
            ...
            className="px-8 py-4 bg-white text-slate-900 border-2 border-slate-900 rounded-2xl font-black uppercase text-lg tracking-widest hover:scale-105 transition-transform flex items-center gap-3 shadow-2xl"
          >
            <BookOpen size={20} /> Refleksjon
          </button>
          <button
            onClick={goToScoreboard}
            className="px-8 py-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-lg tracking-widest hover:scale-105 transition-transform flex items-center gap-3 shadow-2xl"
          >
      ...
      {gameStatus === "reflection" && (
        <div className="absolute inset-0 bg-indigo-600 z-50 flex flex-col items-center justify-center text-white animate-in fade-in duration-500 p-12">
          <div className="max-w-3xl w-full space-y-8 text-center">
            ...
            <div className="pt-8">
              <p className="text-sm font-black uppercase tracking-[0.3em] opacity-60 mb-6">
                Diskuter med sidemannen
              </p>
              <button
                onClick={goToScoreboard}
                className="px-12 py-5 bg-white text-indigo-600 rounded-2xl font-black uppercase text-xl tracking-widest hover:scale-105 transition-transform flex items-center gap-3 mx-auto shadow-2xl"
              >
      ...
      {gameStatus === "scoreboard" && (
        <div className="absolute inset-0 bg-slate-900/95 backdrop-blur-xl z-50 flex flex-col items-center justify-center text-white animate-in slide-in-from-bottom p-8">
            ...
            {session?.config?.playMode === "teams"
              ? TEAMS.map((team) => {
                  ...
                  .map((team, i) => (
                    <div
                      key={team.id}
                      className="flex justify-between items-center bg-white/10 p-4 rounded-2xl border border-white/5 animate-in slide-in-from-bottom-2"
                      style={{ animationDelay: `${i * 100}ms` }}
                    >
                      <div className="flex items-center gap-4">
                        <span
                          className={`font-black text-xl ${i === 0 ? "text-yellow-400" : "text-slate-400"}`}
                        >
                          #{i + 1}
                        </span>
          ...
          <button
            ...
            className="mt-12 px-10 py-4 bg-white text-slate-900 rounded-2xl font-black uppercase text-xl tracking-widest hover:scale-105 transition-transform flex items-center gap-3"
          >

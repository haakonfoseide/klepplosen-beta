...
export const MathHuntGenerator: React.FC<MathHuntGeneratorProps> = ({
  t,
  language,
  currentUser,
}) => {
  const [step, setStep] = useState<"config" | "lobby" | "active" | "summary">(
    "config",
  );
  ...
  const subscribeToPlayers = useCallback((sessionId: string) => {
    const channel = supabase
      .channel(`mathhunt_lobby_${sessionId}`)
      .on(
        "postgres_changes",
        {
          event: "*",
        ...
        <div className="space-y-6">
          <div className="text-center space-y-2">
            ...
            <p className="text-slate-400 font-medium text-sm">
              Adaptiv mengdetrening med AI-støtte
            </p>
          </div>
        ...
        <div className="absolute top-8 left-8 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">
              Tema: {session?.config?.topic} • {session?.config?.grade}
            </p>
          </div>
        </div>
              ...
              {players.length === 0 ? (
                ...
                <div className="grid grid-cols-1 gap-3">
                  {players.map((p) => (
                    <div
                      key={p.id}
                      className="flex items-center gap-4 p-4 bg-white rounded-2xl shadow-sm border border-slate-100 animate-in slide-in-from-right-4"
                    >
        ...
        <div className="flex justify-between items-center bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                Tema: {session?.config?.topic} • PIN: {session?.pin_code}
              </p>
            </div>
          </div>
          <div className="flex gap-4">
            <TimerComponent t={t} />
            <Button variant="secondary" onClick={handleEndGame} icon={X}>
              Avslutt
            </Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {players.map((p) => {
            ...
            return (
              <div
                key={p.id}
                className={`p-4 rounded-2xl border-2 flex flex-col gap-3 transition-all ${statusColor}`}
              >
                <div className="flex justify-between items-center">
                  <span className="font-black truncate">{p.nickname}</span>
                  <StatusIcon
                    size={16}
                    className={p.team === "yellow" ? "animate-spin-slow" : ""}
                  />
                </div>
      ...
      <div className="max-w-4xl mx-auto space-y-8 animate-in slide-in-from-bottom-8">
        <div className="text-center space-y-4">
          ...
          <p className="text-slate-500 font-medium text-lg">
            Klassen løste totalt{" "}
            <span className="font-black text-indigo-600">{totalSolved}</span>{" "}
            oppgaver!
          </p>
        </div>

        <div className="bg-white p-8 rounded-[2rem] shadow-xl border border-slate-100">
          <h3 className="text-lg font-black uppercase tracking-widest text-slate-400 mb-6 text-center">
            Resultattavle
          </h3>
          <div className="space-y-3">
            {sortedPlayers.map((p, i) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center font-black text-sm ${i === 0 ? "bg-yellow-100 text-yellow-700" : i === 1 ? "bg-slate-200 text-slate-700" : i === 2 ? "bg-amber-100 text-amber-800" : "bg-white text-slate-400"}`}
                  >
                    {i + 1}
                  </div>
